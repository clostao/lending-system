...
describe("One market", function () {
  ...
  it("Add market to protocol controller", async function () {
    ...
    expect(
      (await protocolContract.availableMarkets(debtToken.address)).isListed
    ).to.be.true;
    expect(
      (await protocolContract.availableMarkets(debtToken.address))
        .underlyingAsset
    ).to.be.equal(underToken.address);
    expect(
      (await protocolContract.availableMarkets(debtToken.address))
        .collateralFactor.numerator
    ).to.be.equal(80);
    expect(
      (await protocolContract.availableMarkets(debtToken.address))
        .collateralFactor.denominator
    ).to.be.equal(100);
  });
  ...
  it("Add market, mint and redeem", async function () {
    ...
    await priceOracle.setPrice(debtToken.address, 2, 1);
    await debtToken.mint(1000000);
    expect(await debtToken.balanceOf(signer.address)).to.be.equals(
  ...
  it("Add market, mint and borrow", async function () {
    ...
    await debtToken.mint(1000000);
    expect(await underToken.balanceOf(signer.address)).to.be.equal(0);
    let [solvent, balance] = await protocolContract.getExpectedLiquidity(
      signer.address,
      debtToken.address,
      0,
      800000
    ...
    const [totalBorrowed, collateral] = await debtToken.getAccountSnapshot(
      signer.address
    );
    expect(totalBorrowed).to.be.equals(
      BigNumber.from(800000).mul(100001).div(100000)
    );
  ...
  it("Add market, mint, borrow and repay", async function () {
    ...
    expect(borrowed).to.be.equals(800000);
    expect(await underToken.balanceOf(signer.address)).to.be.equal(800000);
    let [totalBorrowed, collateral] = await debtToken.getAccountSnapshot(
      signer.address
    );
    expect(totalBorrowed).to.be.equals(
      BigNumber.from(800000).mul(100001).div(100000)
    ...
    await debtToken.redeem(
      (await debtToken.balanceOf(signer.address))
        .mul(100000)
        .div(100001)
        .add(800000)
    );
